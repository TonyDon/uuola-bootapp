/*
 *  txdnet.cn tonydon
 * 
 */
package com.uuola.webapp.constants;

/**
 *
 * @author txdnet
 */
public class CHAR {

    public static final char CHAR_SLASH = '/';
    public static final char CHAR_BACK_SLASH = '\\';
    public static final char CHAR_COMMA = ',';
    public static final char CHAR_LINE = '-';
    public static final char CHAR_UNDER_LINE = '_';
    public static final char CHAR_VERTICAL_LINE = '|';
    public static final char CHAR_DOT = '.';
    public static final char CHAR_EQUAL = '=';
    public static final char CHAR_AND = '&';
    public static final char CHAR_PERCENT = '%';
    public static final char CHAR_AT = '@';
    public static final char CHAR_WELL='#';
    public static final char CHAR_LF = '\n';
    public static final char CHAR_CR = '\r';
    public static final char CHAR_QUESTION='?'; 
    public static final String STR_SLASH = "/";
    public static final String STR_BACK_SLASH = "\\";
    public static final String STR_COMMA = ",";
    public static final String STR_LINE = "-";
    public static final String STR_UNDER_LINE = "_";
    public static final String STR_VERTICAL_LINE = "|";
    public static final String STR_DOT = ".";
    public static final String STR_COLON = ":";
    public static final String STR_EQUAL = "=";
    public static final String STR_AND = "&";
    public static final String STR_PERCENT = "%";
    public static final String STR_AT = "@";
    public static final String STR_WELL="#";
    public static final String STR_QUESTION="?";
    public static final String STR_EMPTY = "";
    public static final String STR_LFCR = "\n\r";
    public static final String STR_CRLF = "\r\n";
    public static final String LEFT_PADDING_9 = "%09d";
    public static final String LEFT_PADDING_3 = "%03d";
    public static final String STR_SPACE = " ";
}
